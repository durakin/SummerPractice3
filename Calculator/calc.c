...
int to_postfix_notation(char *infix_notation, char *postfix_notation) {
  char stackbuff[strlen(infix_notation)];
  int stackpointer = -1;

  for (int i = 0; i < strlen(infix_notation); i++) {
    char current = infix_notation[i];
    if (isdigit(current)) {
      i = last_digit_index(infix_notation, i,
    ...
    if (get_priority(current) != -1) {
      //TODO: unary minus
      while (stackpointer >= 0 && (get_priority(stackbuff[stackpointer])
...
double calculate_postfix_notation(char *postfix_notation) {
  int length = strlen(postfix_notation);
  double stackbuff[length];
  int stackpointer = -1;
  int counter = 0;
